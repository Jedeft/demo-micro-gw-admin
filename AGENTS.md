...
| 覆盖率 | **自检目标**：核心业务逻辑（`services` 层）行覆盖率不低于 **90%**。若因客观原因无法达到，必须在自我代码审查中明确说明理由。 |
...
- **必须执行 (ALWAYS)**:
    ...
    - 当前项目打印日志必须使用github.com/Jedeft/xuanwu/log模块，并且日志打印不管任何级别一定要有xuanwu的tracer可追踪。
    - **每次完成一个功能或模块的代码编写后，必须立即进行一次完整的自我代码审查（Self-Review）**。审查必须在提交代码前完成，审查内容至少包括：
        *   **逻辑与边界**：检查是否存在逻辑漏洞、未处理的边界条件（如空值、切片越界）。
        *   **错误处理**：确保所有 `error` 都被妥善处理或明确忽略（`_`）。
        *   **架构合规**：确认代码严格遵守“依赖方向”（Handler → Service → Repository）和“禁止跨层调用”的约束。
        *   **测试覆盖**：检查是否为核心业务逻辑补充了单元测试，并通过 `go test` 验证。
    - 自我审查的结果（发现的问题与修复记录）必须记录在当次任务的 session 摘要（`.checkpoints/sessions/`）中。
